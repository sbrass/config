...
you should place your code here."
  ...
  (with-eval-after-load 'org
    ...
    (setq org-ref-default-bibliography '("~/Documents/references.bib")
          org-ref-pdf-directory "~/Documents/Paper"
          org-ref-bibliography-notes "~/Documents/Notes/notes.org")
    (setq org-ref-open-pdf-function
          (lambda (fpath)
            (start-process "zathura" "*helm-bibtex-zathura*" "/usr/bin/zathura" fpath)))

    ;; Set zathura to default viewer for ORG
    (setq org-file-apps
          '((auto-mode . emacs)
            ("\\.mm\\'" . default)
            ("\\.x?html?\\'" . default)
    ...
    ;; Latex Compiler and Processing
    (setq org-latex-compiler "lualatex")

    ;; latexmk is alreay an option for org-latex-pdf-process, but we
    ;; specify the command a little bit more in detail. We do not have to give
    ;; the Bib compiler, because latexmk is intellegent enough to find the
    ;; correct one.
    (setq org-latex-pdf-process
          '("latexmk --shell-escape --interaction=nonstopmode --halt-on-error --lualatex --outdir='%o' %f"))

    ;; Add compiler-specific packages: lualatex
    ;; The language for polyglossia is added by org-latex-guess-polyglossia-language.
    (setq org-latex-default-packages-alist
          '(
            ...
            ("" "fontspec" t
             ("lualatex"))
            ("math-style=ISO, bold-style=ISO, sans-style=italic, nabla=upright, partial=upright" "unicode-math" t
             ("lualatex"))
            ("AUTO" "inputenc" t
             ("pdflatex"))
            ("T1" "fontenc" t
    ...
    (setq org-preview-latex-process-alist
          '((dvipng :programs
            ...
            (imagemagick :programs
                         ("latex" "convert")
                         :description "pdf > png" :message "you need to install the programs: lualatex and imagemagick." :use-xcolor t :image-input-type "pdf" :image-output-type "png" :image-size-adjust
                         (1.0 . 1.0)
                         :latex-compiler
    ...
    (setq org-capture-templates
          '(("t" "Todo" entry (file+headline "~/Documents/TODO.org" "Tasks")
             "* TODO %?\n  %i\n  %a")
            ("j" "Journal" entry (file+datetree "~/Documents/notes.org")
             "* %?\nEntered on %U\n  %i\n  %a")
